...
public interface PredicateStepThrowable<T extends Throwable> extends PredicateStep<PredicateStepThrowable<T>, T> {
    ...
    @Override
    default PredicateAssertorStepThrowable<T> and() {
        return () -> HelperStep.and(this.getStep());
    }

    @Override
    default PredicateAssertorStepThrowable<T> or() {
        return () -> HelperStep.or(this.getStep());
    }

    @Override
    default PredicateAssertorStepThrowable<T> xor() {
        return () -> HelperStep.xor(this.getStep());
    }

    @Override
    default PredicateAssertorStepThrowable<T> nand() {
        return () -> HelperStep.nand(this.getStep());
    }

    @Override
    default PredicateAssertorStepThrowable<T> nor() {
