 ...
 * </p>
 * <ul>
 * <li>{@link PredicateEnd#isOK}: to get the boolean result of the assertion
 * {@code true} or {@code false}.</li>
 * <li>{@link PredicateEnd#getErrors}: to get the error message (precondition
 * message or message depending of error type).</li>
 * <li>{@link PredicateEnd#get}: to get the result as an {@link Optional}
 * object. The result is set to empty if the assertion failed or if the checked
 * value is {@code null}.</li>
 * <li>{@link PredicateEnd#getNullable}: to get the result even if value is
 * {@code null}.</li>
 * <li>{@link PredicateEnd#orElse}: to get the value if not {@code null},
 * otherwise returns the default value.</li>
 * <li>{@link PredicateEnd#orElseThrow}: to throw an exception if assertion is
 * false, or to get the checked value otherwise.</li>
 * <li>{@link PredicateEnd#asResult}: to get the result as a {@link Result}
 * object. The result is set to empty only if the assertion failed.</li>
 * <li>{@link PredicateEnd#asDefault}: to get the result as a {@link Default}
 ...
 */
@FunctionalInterface
public interface PredicateStep<S extends PredicateStep<S, T>, T> extends PredicateEnd<S, T>, PredicateOperatorAnd<S, T>,
        PredicateOperatorNand<S, T>, PredicateOperatorNor<S, T>, PredicateOperatorOr<S, T>, PredicateOperatorXor<S, T> {
     ...
     */
    default PredicateEnd<S, T> that(final T object) {
        return () -> HelperAssertor.object(this.getStep(), object);
    }
}